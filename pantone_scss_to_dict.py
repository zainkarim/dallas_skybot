...
# Regular expression pattern to match color name and value
pattern = r"\s*([^:]+):\s*#([a-fA-F0-9]+),"

# Initialize an empty dictionary to store the color names and values
pantone_color_names = {}

# Extract color name and value pairs from the .scss data using regular expression
matches = re.findall(pattern, scss_data)

# Create the dictionary
for color_name, color_value in matches:
    pantone_color_names[color_name] = "#" + color_value

# Define the output file name
output_file = "pantone_colors.json"

# Write the dictionary to a JSON file
with open(output_file, "w") as file:
    json.dump(pantone_color_names, file, indent=4)

print(f"Successfully saved the dictionary to {output_file}.")